...
from crewai.tools import BaseTool  # For custom tools
...
def test_model(model_name):
    ...
    try:
        llm = OllamaLLM(model=model_name, temperature=0.2,base_url="http://localhost:11434")
...
class SearchPatentsTool(BaseTool):
    ...
    def _run(self, query: str, top_k: int = 20) -> str:
        ...
        try:
            ...
            # Format results as a string for LLM
            formatted_results = []
...
def create_patent_analysis_crew(model_name="deepseek-r1:1.5b"):
    ...
    task2 = Task(
        description="""
        Using the research plan, retrieve patents related to renewable energy from the last 3 years.
...
def run_patent_analysis(research_area="Lithium Battery", model_name="llama3"):
    ...
    try:
        ...
        return (
            ...
            + "1. Make sure Ollama is running: 'ollama serve' (Docker for you)\n"
            + "2. Pull a compatible model: 'ollama pull deepseek-r1:1.5b' or choose another listed model\n"
